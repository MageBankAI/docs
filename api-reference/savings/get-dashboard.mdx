...
- The dashboard only includes active investments; completed or cancelled investments are not included.
- Interest calculations are performed in real-time based on the current platform interest rate.
- The `yearProjection` value shows the estimated total value after one year if all current investments remain active at the current interest rate.
...
- If there are no active investments, the `agents` array will be empty and totals will be zero.
