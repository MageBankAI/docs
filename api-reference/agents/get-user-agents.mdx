...
- The user ID can be in the short format (e.g., `user_piXARPaD2jefNBGxzb84Qd`) or UUID format.
- If the user has no agents, an empty array will be returned.
- Results are not paginated by default. If the user has many agents, consider implementing client-side pagination for better performance.