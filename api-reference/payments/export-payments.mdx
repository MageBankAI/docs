...
Exports payment data in CSV, XLSX, or PDF format based on the specified date range. This endpoint is useful for accounting, reporting, and data analysis purposes.
...
- Each export format has specific advantages:
  - **CSV**: Simple text format compatible with most spreadsheet applications
  - **XLSX**: Excel format with advanced formatting and calculation capabilities
  - **PDF**: Formatted document suitable for sharing and printing
- Export files include the following information:
  - Payment ID
  - Payment Name
  ...
  - Tags
- The maximum number of payments that can be exported in a single request is 10,000. For larger datasets, use multiple requests with narrower date ranges.
- Export processing may take longer for large datasets, especially in PDF format.